...
contract Medblock {
    ...
    function add_Person(string memory _name, uint _age, uint _phoneno, string memory _gender, uint _designation, string memory _hash) public returns(string memory){
        ...
        if(_designation == 0){
            ...
            doctorInfo[addr].phoneno = _phoneno;
            doctorInfo[addr].gender = _gender;
    ...
    function get_patient(address addr) view public returns (string memory , uint, uint, string memory, uint[] memory , address, string memory ){
        // if(keccak256(patientInfo[addr].name) == keccak256(""))revert();
        return (patientInfo[addr].name, patientInfo[addr].age, patientInfo[addr].phoneno, patientInfo[addr].gender, patientInfo[addr].diagnosis, Empty[addr], patientInfo[addr].record);
    }

    function get_doctor(address addr) view public returns (string memory , uint, uint, string memory){
        // if(keccak256(doctorInfo[addr].name)==keccak256(""))revert();
        return (doctorInfo[addr].name, doctorInfo[addr].age, doctorInfo[addr].phoneno, doctorInfo[addr].gender);
    }
    ...
    function Details_deta(address paddr, uint _diagnosis, string memory  _hash) public {
